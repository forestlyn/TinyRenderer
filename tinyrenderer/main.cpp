...
void triangle(Vec3f *pts, Vec2i *uvs, float *zbuffer, TGAImage &image, float *intensities)
{
	...
	for (float x = minx; x <= maxx; x += 1)
	{
		for (float y = miny; y <= maxy; y += 1)
		{
			...
			if (zbuffer[int(x + y * width)] < z)
			{
				...
				zbuffer[int(x + y * width)] = z;
				TGAColor color;
				Vec2i uv = uvs[0] * res.x + uvs[1] * res.y + uvs[2] * res.z;
				float intensity = intensities[0] * res.x + intensities[1] * res.y + intensities[2] * res.z;
...
int main(int argc, char **argv)
{
	...
	Perspective[3][2] = -1. / (camera - center).norm();
	...
	for (int i = 0; i < model->nfaces(); i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			world_verts[j] = v;
			Vec3f temp = m2v(view * (Perspective * ModelView * v2m(v)));
			pts[j] = Vec3f(int(temp.x), int(temp.y), temp.z);
			uvs[j] = model->uv(i, j);
			intensities[j] = model->norm(i, j) * lightdir;
		}
		Vec3f n = (world_verts[2] - world_verts[0]) ^ (world_verts[1] - world_verts[0]);
		n.normalize();
		triangle(pts, uvs, zbuffer, image, intensities);
	}
	printf("end");
